...
template <typename T>
Matrix<T>::Matrix(const std::size_t rows, const std::size_t cols) : m_rows(rows), m_cols(cols), m_data(m_rows, std::vector<T>(m_cols, T{}))
{
}

template <typename T>
Matrix<T>::Matrix(const std::size_t rows, const std::size_t cols, const T value) : m_rows(rows), m_cols(cols), m_data(m_rows, std::vector<T>(m_cols, value))
{
}
...
Matrix<T> Matrix<T>::operator+(const Matrix<T> &rhs) const
{
    ...
    if ((this->m_rows == rhs.m_cols) && (this->m_cols == rhs.m_cols))
    {
        for (std::size_t i = 0; i < m_rows; i++)
        {
            std::transform(m_data[i].begin(), m_data[i].end(), rhs.m_data[i].begin(), result.m_data[i].begin(), std::plus<T>());
        }
...
Matrix<T> Matrix<T>::operator-(const Matrix<T> &rhs) const
{
    ...
    if ((this->m_rows == rhs.m_cols) && (this->m_cols == rhs.m_cols))
    {
        for (std::size_t i = 0; i < m_rows; i++)
        {
            std::transform(m_data[i].begin(), m_data[i].end(), rhs.m_data[i].begin(), result.m_data[i].begin(), std::minus<T>());
        }
