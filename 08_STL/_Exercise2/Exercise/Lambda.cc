...
int main()
{
    ...
    const auto has_no_even =
        std::none_of(my_vector.begin(), my_vector.end(), [](const auto value){return (value % 2) != 0;});
    ...
    // Exercise 2
    std::transform(my_vector.begin(), my_vector.end(), my_result.begin(), [](const auto value){return value * value;});
