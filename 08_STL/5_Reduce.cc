...
int main()
{
    ...
    const auto sum1 = std::reduce(std::execution::seq,my_vector.begin(), my_vector.end(), 0);
    ...
    const auto sum2 = std::reduce(std::execution::par, my_vector.begin(), my_vector.end(), 0);
    const auto end_time2 = ClockType::now();
    const auto elapsed_time2 = std::chrono::duration_cast<ClockRes>(end_time2-start_time2).count();
    std::cout << "Elapsed Time2: " << elapsed_time2 << std::endl;
    std::cout << "Sum2: " << sum2 << std::endl;

    //Übergabe binäre Funktion
    const auto sum3 = std::reduce(std::execution::seq,my_vector.begin(), my_vector.end(), 0, func1);
